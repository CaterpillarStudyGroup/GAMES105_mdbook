...
从目前的统计来看，随机方法的成熟与硬件能力的提供，RL学习方法趋势从端到端->分层->端到端。  
分层控制的方法中，大家更倾向于把RL用在中层控制而不是高层控制。这一点我是可以理解的。因为中层控制比高层控制更需要临场应变能力，也更缺数据（不同的机器人做同一件事，可以使用相似的规划，但对应不同的具体执行，或者说高层控制更有通用性，更容易实现数据复用）。
