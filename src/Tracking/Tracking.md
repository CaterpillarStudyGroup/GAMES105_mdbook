...
**与 PD 控制的关系**：
- 轨迹优化输出**目标轨迹** \((q^*, \dot{q}^*)\) → PD 控制的目标
- 轨迹优化输出**控制轨迹** \((\tau^*)\) → PD 控制的前馈项
...
| 概念 | 符号 | 是什么 | 优化前/后 | 角色 |
|------|------|--------|----------|------|
| **参考轨迹** | \((q^{\text{ref}}, \dot{q}^{\text{ref}})\) | 动捕/运动学方法/动画师提供的原始轨迹 | 优化**前** | 轨迹优化的**输入** |
| **状态轨迹** | \((q, \dot{q})\) | 角色在每一帧的实际状态（位置 + 速度） | 通用术语 | 可指任何轨迹 |
| **目标轨迹** | \((q^*, \dot{q}^*)\) | 轨迹优化后得到的物理可行轨迹 | 优化**后** | 轨迹优化的**输出** → PD 控制的**目标** |
| **控制轨迹** | \((\tau)\) | 每一帧的关节力矩序列 | 优化**后** | 轨迹优化的**输出** → PD 控制的**前馈项** |
...
> &#x1F4D6; **数学形式化描述**：详见 [数学形式化描述](MathematicalFormulation.md) - 轨迹优化问题的完整数学描述。
