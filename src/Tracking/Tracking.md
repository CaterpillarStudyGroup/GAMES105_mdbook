...
轨迹优化需要**参考轨迹**作为输入，参考轨迹可以来自：

| 输入来源 | 说明 | 特点 |
|---------|------|------|
| **Mocap 数据** | 动作捕捉数据 | 真实自然，但可能物理不可行 |
| **Motion Matching** | 数据库搜索生成的轨迹 | 质量高，但内存占用大 |
| **相位方法（PFNN）** | 相位条件化生成 | 实时生成，但需要训练 |
| **关键帧动画** | 美术手工制作 | 艺术可控，但耗时 |
| **扩散模型** | 概率生成 | 高质量多样性，推理慢 |
...
```mermaid
flowchart LR
    Mocap["Mocap / Motion Matching<br/>/ PFNN / 关键帧"] --> Opt["轨迹优化"]
    Opt --> PD["PD 控制"]
    PD --> Sim["物理仿真"]
```
